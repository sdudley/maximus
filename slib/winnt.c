...
void _fast WinPutc(VWIN *win,byte Char)
{
  ...
  *WinOfs(win,win->row,lastc)=Cell(win->attr, Char);
...
void _fast WinScrl(VWIN *win,int dir,int srow,int erow,int attr)
{
  ...
  if (dir==SCROLL_down)
  {
    memmove(win->rowtable[srow+1] - (win->border == BORDER_NONE ? 0 : sizeof(WNCELL)),
            win->rowtable[srow] - (win->border == BORDER_NONE ? 0 : sizeof(WNCELL)),
            (win->s_width << CELLSHIFT) * (erow-srow));
  ...
  {
    memmove(win->rowtable[srow] - (win->border == BORDER_NONE ? 0 : sizeof(WNCELL)),
            win->rowtable[srow+1] - (win->border == BORDER_NONE ? 0 : sizeof(WNCELL)),
            (win->s_width << CELLSHIFT) * (erow-srow));
...
{
  *WinOfs(win, Row, Col)=Cell(Attr, Char);
...
static void near WinDrawShadow(VWIN *win, unsigned check)
{
  ...
  void pascal _WinBlitzNT(VWIN *win,
                          int row,                  /* starting row (screen) */
                          int col,                  /* starting column (screen) */
                          ...
                          int num_col)              /* number of columns   */
  {
    ...
    origc.X=col + (win->border != BORDER_NONE);
    origc.Y=row + (win->border != BORDER_NONE);

    sr.Top=row;
    sr.Bottom=row + num_row - 1;
    sr.Left=col;
    sr.Right=col + num_col - 1;

    WriteConsoleOutput(hStdout, (PCHAR_INFO)win->buf, c, origc, &sr);
  }
  ...
  for (;x < max;x++)
  {
    ...
    if (flag || win->dleft[x] != -1)
    {
      ...
      if (!Vid_Bios && /*x >= 0 &&*/ x < (sword)win->s_height+(win->border != BORDER_NONE) && win->blitz[x])
      {
        ...
        for (y=x+1; y < max; )
        {
          if ((flag || win->dleft[y] != -1) &&
              (y < (sword)win->s_height+(win->border != BORDER_NONE) && win->blitz[y]))
          {
            ++y;
          }
          else
        ...
        if (y-x==1)
        {
          col = win->s_col + win->dleft[x];
          num_cols = win->dright[x] - win->dleft[x] + 1;
        }
        else
        {
          col=0;
          num_cols = win->s_width;
        }

        _WinBlitzNT(win, x, col, y - x, num_cols);
      ...
      else
      {
        ...
        else
        {
          while (y <= right)
          {
            ...
            {
              chattr=*WinOfs(win,x,y);
              VidPutch(this_row,this_col, CharOf(chattr), AttrOf(chattr));
            }
...
VWIN * _fast WinOpen(int row,int col,int height,int width,int border,int attr,int battr,int flag)
{
  ...
  if ((win->buf=farmalloc((height+1)*(width+2)*sizeof(WNCELL)))==NULL)
  {
    free(win->orig_rt);
    free(win);
    return NULL;
  ...
  if ((win->dleft=win->orig_dl=malloc((height+2)*sizeof(int)))==NULL ||
       (win->dright=win->orig_dr=malloc((height+2)*sizeof(int)))==NULL ||
  ...
  for (x=0;x < height;x++)
    win->rowtable[x]=(char far *)((char far *)win->buf+(width*x*sizeof(WNCELL)));
  ...
  if (flag & WIN_INHERIT) /* Inherit underlying text (transparent) */
  {
    ...
    cDestSize.X = win->b_width;
    cDestSize.Y = win->b_height;

    cDestCoord.X = (win->border != BORDER_NONE);
    cDestCoord.Y = cDestCoord.X;

    sr.Top = cDestCoord.Y;
    sr.Bottom = sr.Top + win->b_height - 1;
    sr.Left = cDestCoord.X;
    sr.Right = sr.Left + win->b_width - 1;

    ReadConsoleOutput(hStdout,
                      (PCHAR_INFO)win->buf,
                      cDestSize,
                      cDestCoord,
                      &sr);
    ...
    for (x=0;x < height;x++)
    {
      for (y=0; y < width; y++)
      {
        word at = VidGetch((byte)(row+x), (byte)(col+y));

        *WinOfs(win,x,y)=Cell((byte)(at >> 8), (byte)at);
      }
...
int main()
{
  ...
  for (i=0; i < 100; i++)
  {
    ...
    sprintf(temp, "This is test %d of window code.  I wonder if it will scroll the screen okay?\r\n", i);
    WinPuts(wscrn, temp);
    WinSync(wscrn, TRUE);
  }
