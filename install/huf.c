...
{
    uint i, k, flags=0, root, pos, size;

    root = make_tree(NC, c_freq, c_len, c_code);
    size = c_freq[root];  putbits(16, size);
    if (root >= NC) {
        count_t_freq();
        root = make_tree(NTT, t_freq, pt_len, pt_code);
        if (root >= NTT) {
            write_pt_len(NTT, TBIT, 3);
        } else {
